...
router.post('/ppus/validaciones', async (ctx) => {
    ...
    if ( !tempregion && !tempCantidadRecorridos && !templistappu) {

        let inputValidarFlota = new InputValidarFlota("", inputParams.region, inputParams.lstPpuRut, inputParams.CantidadRecorridos,"")
        log.trace("inputParameters: " + JSON.stringify(inputValidarFlota))
        ctx.body = await logicTaxis.validarFlota(inputValidarFlota)
