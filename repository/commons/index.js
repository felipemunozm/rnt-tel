...
module.exports = {
    ...
    getServiciosVigentesInscritosPorRutResponsable: (rut_responsable,lstTipoServicios) =>{
        return ibmdb.query("SELECT DISTINCT REGION.ID AS ID_REGION, REGION.NOMBRE AS REGION, T_VEHICULO_SERV.NOMBRE || ' ' || MODALIDAD.NOMBRE || ' ' || T_SERV_AREA.NOMBRE AS TIPO_SERVICIO, VISTA.IDENT_SERVICIO AS FOLIO, RESPONSABLE.NOMBRE AS NOMBRE, RESPONSABLE.RUT AS RUT_RESPONSABLE " +
        ...
        "INNER JOIN NULLID.RNT_RESPONSABLE_SERVICIO AS RS ON RS.ID = VISTA.ID_TIPO_SERVICIO " +
    ...
    getServiciosVigentesInscritosPorRutResponsableAndRutMandatario: (rut_responsable, rut_mandatario, lstTipoServicios) =>{
        return ibmdb.query("SELECT DISTINCT REGION.ID AS ID_REGION, REGION.NOMBRE AS REGION, T_VEHICULO_SERV.NOMBRE || ' ' || MODALIDAD.NOMBRE || ' ' || T_SERV_AREA.NOMBRE AS TIPO_SERVICIO, VISTA.IDENT_SERVICIO AS FOLIO, RESPONSABLE.NOMBRE AS NOMBRE, RESPONSABLE.RUT AS RUT_RESPONSABLE, MANDATARIO.RUT AS RUT_MANDATARIO " +
        "FROM NULLID.RNT_SERVICIO AS VISTA " +
        "INNER JOIN NULLID.RNT_TIPO_SERVICIO AS T_SERV ON T_SERV.ID = VISTA.ID_TIPO_SERVICIO " +
        "INNER JOIN NULLID.RNT_SERVICIO_REPRESENTANTE SR ON VISTA.ID = SR.ID_SERVICIO " +
        "INNER JOIN NULLID.RNT_REPRESENTATE_LEGAL AS RL ON SR.ID_REPRESENTANTE_SERVICIO = RL.ID " +
        ...
        "LEFT JOIN NULLID.RNT_TIPO_SERVICIO_AREA AS T_SERV_AREA ON T_SERV.ID_TIPO_SERVICIO_AREA = T_SERV_AREA.ID " +
        "LEFT JOIN NULLID.UTILSQA_REGION AS REGION ON REGION.ID = VISTA.CODIGO_REGION " +
        "WHERE RESPONSABLE.RUT = ? AND MANDATARIO.RUT = ? AND VISTA.ACTIVO = 1 AND RESPONSABLE.TIPO_PERSONA = 1 AND MAND.AUTORIZADO_TRAMITES = 1 AND T_SERV.ID IN (" + lstTipoServicios.toString() + ")",[rut_responsable, rut_mandatario])
    },
        //psalas
