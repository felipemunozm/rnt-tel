...
/**
 * @param {string} tipoVehiculoLargo  como texto largo para ser reducido al tipo interno
 */
const getTipoVehiculo = (tipoVehiculoLargo) => {
    switch (tipoVehiculoLargo) {
        case 'MINIBUS':
        case 'MICROBUS':
            return "MINIBUS"
        ...
        case 'AUTOMOVIL':
            return "AUTOMOVIL"
        default:
            throw new Error(`Tipo vehiculo '${tipoVehiculoLargo}' no reconocido`)
    }
}
...
module.exports = {
    ...
    getAutorizadoPorEmpresaAndSolicitanteInscripcionServicio: (id_region, rut_representante, rut_solicitante, idtramite) => {
        //crear logica de respuesta
        //1 Empresa indicada no se encuentra habilitada para realizar tramites en linea
        log.debug(id_region, rut_representante, rut_solicitante, idtramite)
        let hab_empresa_tram = ibmdb.query("select count(*) as total FROM   tel.TEL_PERSONA per 	 " +
    ...
    findAntiguedadMaximaByTipoVehiculo: (tipoVehiculo, folio, codigoRegion, tipoIngreso) => {
        let idreglamentacion = -1
        let ModalidaddelServicio = ibmdb.query("select CATEGORIA, TIPOSERVICIO, MODALIDAD,TIPOVEHICULO  from NULLID.RNT_STAT_SERVICIOS_VIEW SV where sv.IDENT_SERVICIO=? and sv.CODIGO_REGION= ? and sv.CATEGORIA<>'PRIVADO' ", [folio, codigoRegion])

        if (tipoVehiculo == "AUTOMOVIL") {

            if (ModalidaddelServicio[0].MODALIDAD == "TAXI COLECTIVO" && tipoIngreso == "RENUEVA TU TAXI") {
            ...
            let anioPorTipoDeVeviculo = ibmdb.query(query, [idreglamentacion, idtipoVehiculoFiltrado, codigoRegion])
