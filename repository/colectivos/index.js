    ...
    getAutorizacionPorEmpresaAndPersonaTramiteInscripcionTaxiColectivo: (id_region, rut_empresa, rut_solicitante, idtramite) => {
        ...
        if (hab_empresa_tram[0].TOTAL > 0) {
            ...
            let hab_persona_tram = ibmdb.query("select count(*) as total " +
                ...
                "where  per.RUT = ? AND per2.RUT = ?  ", [rut_empresa, rut_solicitante])
            ...
            if (hab_persona_tram[0].TOTAL > 0) {
                ...
                let hab_persona_region = ibmdb.query(query, [rut_empresa, rut_solicitante])
                ...
                if (hab_persona_region[0].RESUL == 1) {
                    ...
                    let query = "select tram.id as id_tram " +
                        ...
                        "INNER JOIN NULLID.RNT_TRAMITE tram                    ON tram.id = ?  and tram.ID= autt.ID_TRAMITE " +
                        "where aut.CODIGO_REGION   = ? and per.RUT = ? AND per2.RUT = ?  "
                    let hab_persona_tramite = ibmdb.query(query, [idtramite, id_region, rut_empresa, rut_solicitante])
                    if (hab_persona_tramite.length > 0) {
                        ...
                        if (hab_persona_tramite[0].TOTAL > 0) {
                            return ibmdb.query("select  aut.ID_CATEGORIA ,aut.ID_TIPO_SERVICIO ," +
                                ...
                                "INNER JOIN NULLID.RNT_TRAMITE tram                       ON tram.id = ?  and tram.ID= autt.ID_TRAMITE " +
                                "WHERE  current date between aut.FECHA_VIGENCIA_DESDE and aut.FECHA_VIGENCIA_HASTA " +
                                "and aut.CODIGO_REGION   = ? and per.RUT = ? AND per2.RUT = ?  ", [idtramite, id_region, rut_empresa, rut_solicitante])
                        }
                        else {
            ...
            if (hab_persona_tram[0].TOTAL > 0) {
                ...
                if (hab_persona_region[0].RESUL == 1) {
                    ...
                    let query = "select tram.id as id_tram " +
                        ...
                        "INNER JOIN NULLID.RNT_TRAMITE tram                    ON tram.id = ?  and tram.ID= autt.ID_TRAMITE " +
                        "where aut.CODIGO_REGION   = ? and per.RUT = ? AND per2.RUT = ?  "
                    let hab_persona_tramite = ibmdb.query(query, [idtramite, id_region, rut_representante, rut_solicitante])
                    if (hab_persona_tramite.length > 0) {
                        //5 Ha expirado su vigencia de habilitacion para realizar entre tramite. Dirijase a la Seremitee correspondiente para actualizar su documentacion
                        let query = "select count(1) as total " +
                            "FROM    tel.TEL_PERSONA per 	" +
                        ...
                        if (hab_persona_tramite[0].TOTAL > 0) {
                            return ibmdb.query("select  aut.ID_CATEGORIA ,aut.ID_TIPO_SERVICIO ," +
                                ...
                                "INNER JOIN NULLID.RNT_TRAMITE tram                       ON tram.id = ?  and tram.ID= autt.ID_TRAMITE " +
                                "WHERE  current date between aut.FECHA_VIGENCIA_DESDE and aut.FECHA_VIGENCIA_HASTA " +
                                "and aut.CODIGO_REGION   = ? and per.RUT = ? AND per2.RUT = ?  ", [idtramite, id_region, rut_representante, rut_solicitante])
                        }
                        else {
                            return { mensaje: 'Ha expirado su vigencia de habilitacion para realizar este tramite. Dirijase a la Seremitt correspondiente para actualizar su documentacion', estado: 'RECHAZADO' }
